...
const API_BASE = '/admin/api/coins';
...
async function fetchRecords() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Failed to load coin consumption: ${response.status}`);
...
function bindEvents() {
  ...
  document.querySelector('#coin-refresh')?.addEventListener('click', () => fetchRecords());

    document.querySelector('#coin-export')?.addEventListener('click', async () => {
      const token = ensureToken();
      if (!token) return;
      try {
        const params = new URLSearchParams();
        ...
        let filename = 'coin_consumption_export.csv';
