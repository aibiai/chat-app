...
const localeSelect = document.querySelector('#help-locale');
const textArea = document.querySelector('#help-text');
const btnReset = document.querySelector('#help-reset');
const btnSave = document.querySelector('#help-save');
const statusEl = document.querySelector('#help-status');
...
async function loadHelp() {
  ...
  try {
    ...
    const sec = data?.data?.sections?.help || { title: '', content: [] };
...
async function saveHelp() {
  ...
  setStatus('保存中...');
  try {
    const body = JSON.stringify({ sections: { help: { title: '', content: paragraphs } } });
...
function bindEvents() {
  localeSelect.addEventListener('change', () => loadHelp());
  btnReset.addEventListener('click', () => loadHelp());
  btnSave.addEventListener('click', () => saveHelp());
