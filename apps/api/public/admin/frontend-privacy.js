...
const localeSelect = document.querySelector('#privacy-locale');
const textArea = document.querySelector('#privacy-text');
const btnReset = document.querySelector('#privacy-reset');
const btnSave = document.querySelector('#privacy-save');
...
async function loadPrivacy() {
  ...
  try {
    ...
    const sec = data?.data?.sections?.privacy || { title: '', content: [] };
...
async function savePrivacy() {
  ...
  setStatus('保存中...');
  try {
    const body = JSON.stringify({ sections: { privacy: { title: '', content: paragraphs } } });
...
function bindEvents() {
  localeSelect.addEventListener('change', () => loadPrivacy());
  btnReset.addEventListener('click', () => loadPrivacy());
  btnSave.addEventListener('click', () => savePrivacy());
